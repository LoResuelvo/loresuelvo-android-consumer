...
- **Logout completo (local primero)**: limpiar la sesión local de forma síncrona para que el smart-router navegue a `Welcome` antes de que el handler del botón retorne, y disparar el logout SSO de Auth0 en background (`viewModelScope.launch{}`). El resultado del SSO es fire-and-forget: el usuario ya está deslogueado localmente y puede reintentar el SSO en el próximo login si Auth0 falla. Esto elimina la trampa del browser de Auth0 que se queda abierto sin completar ni cancelar (la app quedaba atrapada en Home con la sesión local viva). El token SDK de Auth0 puede quedar vivo hasta el próximo login; eso no es un riesgo de seguridad local porque el smart-router verifica la sesión de `EncryptedSharedPreferences`, no el estado del SDK.
